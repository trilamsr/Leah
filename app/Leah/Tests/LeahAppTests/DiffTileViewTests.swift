...
final class DiffTileViewTests: XCTestCase {
  ...
  func testParsesAddedLineAsGoldHairline() throws {
    ...
    XCTAssertEqual(DiffTilePayload.LineKind.added.hairlineColor, CodeDiffPalette.champagneGold)
  }

  func testParsesRemovedLineAsRedAlertHairline() throws {
    // Spec §10.1 item 13 — removed gutter glyph color #C8434F (red-alert),
    // distinct from oxblood (#7A1F2B) used for line-background fill.
    let p = try DiffTilePayload(envelope: envelope(unified: "@@ -1,1 +1,1 @@\n-removed\n"))
    ...
    XCTAssertEqual(DiffTilePayload.LineKind.removed.hairlineColor, CodeDiffPalette.redAlert)
  }
