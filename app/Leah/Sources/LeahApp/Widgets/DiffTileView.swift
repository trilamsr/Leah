...
// +lines = champagne-gold left hairline; −lines = red-alert (`Color.leahRedAlert`).
...
public struct DiffTilePayload {
  ...
  public enum LineKind: Equatable {
    ...
    public var hairlineColor: Color? {
      switch self {
      case .added:    return LeahPalette.champagneGold
      case .removed:  return LeahPalette.redAlert
      default:        return nil
      }
    }
  }
...
public struct DiffTileView: View {
  ...
  public var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      header
      Rectangle().fill(LeahPalette.hairline).frame(height: 1)
      ScrollView(.vertical) {
    ...
    .background(LeahPalette.obsidian1)
    .cornerRadius(12)
    .overlay(
      RoundedRectangle(cornerRadius: 12).strokeBorder(LeahPalette.hairline, lineWidth: 1)
    )
  }
  ...
  private func textColor(for kind: DiffTilePayload.LineKind) -> Color {
    switch kind {
    case .added:   return LeahPalette.ivory
    case .removed: return LeahPalette.ivory
    case .context: return LeahPalette.textMuted
    case .hunkHeader: return LeahPalette.textMuted
    }
  }
}
