...
- **CI/test failure → root cause only.** When a test fails or CI breaks, reproduce locally (`go test -race -count=N`), identify the underlying race/timing/logic bug, fix the invariant. Do NOT revert the suspect change, bump the timeout, add retry-loop, or skip the test. A revert is acceptable only when the change itself is wrong. PR #442 surfaced this — operator rejected a SafeGo revert applied to silence a flake; real fix was a deterministic gate in the test. Cross-ref operator-personal `feedback_no_quick_fixes.md`.
