...
Code-writing subagent for leah. Extends `CLAUDE.md` — does NOT restate it. Substitute `<VARS>` then paste into Task dispatch.
...
- LOAD-BEARING CARVE-OUT (NEVER auto-skip): diffs touching `internal/{adapters,audit,backup,brief,budget,costview,ctxmgr,daemonloop,dispatcher,embed,ghclient,intent,memory,notify,obs,operatormodel,patterns,persona,reasoner,regattaclient,reviewer,selflearn,voice,watchdog,web}/`, `cmd/`, `CLAUDE.md`, `scripts/check-*.sh`, `.github/workflows/*`, `docs/engineer/{specs,dispatch-templates}/*.md` require mandatory independent review even on `[DOCS]` / `[CI]` release-notes.
