...
2. **No closed-loop learning**: dispatch templates calcify as prose; SQLiteEngine.RecordFeedback writes orphan rows; memory dir is operator-readable, never injected at dispatch.
...
- Budget: ≤$5/wk (fits LEAH_BUDGET_DOLLARS).
- ROI: HIGHEST. Without this every other improvement is uncalibrated.
...
- **Bug fix dependency**: wire `SQLiteEngine.RecordFeedback` orphan-row issue (PR #140 review carry-over) into ranker FIRST.
...
### S8 — Knowledge graph wiring into engine (carry-over)
...
- ROI: HIGH — knowledge graph is currently shipped vapor.
...
- Expose `leah_get_memory_rule`, `leah_search_audit`, `leah_dispatch_status`, `leah_self_build_status` as MCP tools.
- Wrap SelfBuild in A2A 1.0 agent-card (task delegation + verification via attestation gate).
...
Phase 1 (this week — eval + obs scaffolding, parallel where file-disjoint):
- S1 eval pipeline
...
- S4 Thompson sampling + change-point (depends on S1 for eval)
- S5 reflexion + tournament (depends on S2 for scorecard rows)
- S8 knowledge-graph wiring (depends on nothing — clearable carry-over)
