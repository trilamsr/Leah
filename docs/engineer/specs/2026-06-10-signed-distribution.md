...
All three are file-disjoint and can land in parallel (README.md "file-disjoint code PRs parallelize up to 6"). The spec itself serializes per the spec-PR rule. **No shared helpers across waves:** `scripts/release/` is partitioned by filename per the wave-table above — W141 owns `notarize.sh`/`verify-signed.sh`/`import-cert.sh`, W142 owns `appcast.sh`/`sign-appcast.sh`/`sign-sha256sums.sh`, W143 owns `update-brew-formula.sh`. No cross-wave function imports; duplicate three similar lines if needed (README.md "three similar lines beat a premature abstraction").
