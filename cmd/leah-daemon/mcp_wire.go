...
func startMCPPublish(ctx context.Context, lg *slog.Logger, registry *obs.Registry, errOut io.Writer) {
	startMCPPublishAt(ctx, lg, registry, errOut, mcp.DefaultPublishSocketPath())
}

func startMCPPublishAt(ctx context.Context, lg *slog.Logger, registry *obs.Registry, errOut io.Writer, sockPath string) {
