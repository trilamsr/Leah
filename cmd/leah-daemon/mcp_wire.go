...
func startMCPPublish(ctx context.Context, lg *slog.Logger, registry *telemetry.Registry, errOut io.Writer) {
...
func startMCPPublishAt(ctx context.Context, lg *slog.Logger, registry *telemetry.Registry, errOut io.Writer, sockPath string) <-chan struct{} {
	done := make(chan struct{})
	telemetry.SafeGo(lg, registry, "mcp-publish", func() {
		defer close(done)
