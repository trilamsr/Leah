...
func main() {
	...
	// Morning-brief task: default = appended to weekly tasks (fires once
	// per week, reuses WeeklyHour). LEAH_BRIEF_DAILY=1 promotes it to the
	// independent daily list so the brief lands every morning instead of
	// once a week. LEAH_BRIEF_HOUR (default 8) gates the daily fire — the
	// brief should not wake the operator at 03:00 if the daemon restarts
	// overnight.
	briefTask := buildBriefTask(sd, rc, os.Stdout)
	if os.Getenv("LEAH_BRIEF_DAILY") == "1" {
		...
		_, _ = fmt.Fprintf(os.Stdout, "leah-daemon: brief = daily @ hour %d\n", loop.DailyHour)
	} else {
