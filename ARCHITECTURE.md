...
│  internal/tts/{elevenlabs,apple} + classifier.go + provider.go      │
│  internal/voice/ — wake-word + VAD + suppression                    │
...
### New IPC frame kinds (`internal/ipc/frame.go`)
...
- KG citation join (`internal/knowledge`) runs inside the answer-engine streaming path; citations stream alongside Reasoner tokens.
