...
func TestPushSource_Run_EmitsTypedPayload(t *testing.T) {
	...
	got := rec.snapshot()
	if got[0].Kind != "reminders.store_changed" {
		t.Fatalf("kind=%q want reminders.store_changed", got[0].Kind)
	}
	if _, ok := got[0].Payload.(obs.RemindersStoreChangedEvent); !ok {
		t.Fatalf("payload=%T want obs.RemindersStoreChangedEvent", got[0].Payload)
	}
}

// Reminders.app + CloudKit sync writes a WAL-append burst per logical change —
// burst inside the debounce window must collapse to one downstream signal.
