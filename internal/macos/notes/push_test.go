...
func TestPushSource_Run_EmitsTypedPayload(t *testing.T) {
	...
	got := rec.snapshot()
	if got[0].Kind != "notes_changed" {
		t.Fatalf("kind=%q want notes_changed", got[0].Kind)
	}
	if _, ok := got[0].Payload.(obs.NotesChangedEvent); !ok {
		t.Fatalf("payload=%T want obs.NotesChangedEvent", got[0].Payload)
	}
}

// Notes.app + CloudKit sync writes a WAL-append burst per logical change —
