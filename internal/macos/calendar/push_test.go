...
func TestPushSource_Run_EmitsTypedPayload(t *testing.T) {
	...
	got := rec.snapshot()
	if got[0].Kind != "calendar.store_changed" {
		t.Fatalf("kind=%q want calendar.store_changed", got[0].Kind)
	}
	if _, ok := got[0].Payload.(obs.CalendarStoreChangedEvent); !ok {
		t.Fatalf("payload=%T want obs.CalendarStoreChangedEvent", got[0].Payload)
	}
}

// EventKit + iCloud sync writes a burst of WAL appends per logical mutation;
// the debounce window must collapse them to a single downstream signal.
...
// Negative Debounce must be treated as zero — a caller passing -1s should not
// suppress every emit forever via underflow comparisons.
func TestPushSource_NegativeDebounce_TreatedAsZero(t *testing.T) {
	t.Parallel()
	w := sqliteopen.NewFakeWALWatcher()
	t.Cleanup(func() { _ = w.Close() })
	...
	clk := &fakeClock{now: time.Unix(1_000_000, 0)}
	rec := &recorder{}
	cancel := runPush(t, w, rec, -time.Second, clk)
	t.Cleanup(cancel)
