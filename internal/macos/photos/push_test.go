...
func TestPushSource_Run_EmitsTypedPayload(t *testing.T) {
	...
	got := rec.snapshot()
	if got[0].Kind != "photos.library_changed" {
		t.Fatalf("kind=%q want photos.library_changed", got[0].Kind)
	}
	if _, ok := got[0].Payload.(obs.PhotosLibraryChangedEvent); !ok {
		t.Fatalf("payload=%T want obs.PhotosLibraryChangedEvent", got[0].Payload)
	}
}

// Photos.sqlite WAL flushes a burst of page-sized appends per new asset row —
// the debounce window must collapse that burst into one downstream signal.
