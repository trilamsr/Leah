...
func TestPushSource_Run_EmitsTypedPayload(t *testing.T) {
	...
	got := rec.snapshot()
	if got[0].Kind != "messages_changed" {
		t.Fatalf("kind=%q want messages_changed", got[0].Kind)
	}
	if _, ok := got[0].Payload.(obs.MessagesChangedEvent); !ok {
		t.Fatalf("payload=%T want obs.MessagesChangedEvent", got[0].Payload)
	}
}

// SQLite fires many WAL appends per logical change — burst inside the debounce
// window must collapse to one downstream signal.
