...
func TestPushSource_Run_EmitsTypedPayload(t *testing.T) {
	...
	testutil.Eventually(t, time.Second, 5*time.Millisecond, func() bool {
		src.Inject(osevent.Event{Kind: osevent.FocusStateChanged})
		return len(rec.snapshot()) >= 1
	})

	got := rec.snapshot()
	if got[0].Kind != "focus.state_changed" {
		t.Fatalf("kind=%q want focus.state_changed", got[0].Kind)
	}
	if _, ok := got[0].Payload.(telemetry.FocusStateChangedEvent); !ok {
		t.Fatalf("payload=%T want obs.FocusStateChangedEvent", got[0].Payload)
	}
}

// pmset / DND-Assertions can re-stat the file several times per toggle —
// coalesce the burst into one downstream signal inside the debounce window.
