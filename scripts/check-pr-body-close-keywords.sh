...
#
# Ported from regatta @ 2026-06-09 (wave 9-G3). Catches the trap
# where PR titles like "[FIX] foo (closes #986 #987 #988)" close #986 only.
#
# Inputs:
...
scan_file() {
  ...
  local file=$2
  [ -f "$file" ] || return 0
  # The keyword set is GitHub's auto-close family; clean form has at most
  # one #N per keyword, OR commas between keywords.
