...
#
# Canonical Reviewer-agent-id allowlist matches leah CLAUDE.md:
#   ^(a[0-9a-f]{16}|cavecrew-reviewer-[a-z0-9-]+)$

rv_decide_verdict() {
  case "$RECOMMENDATION" in
    APPROVE)
      # Automerge guard: agent-written APPROVE + agent-enabled automerge
      # leaves zero operator window between APPROVE-token landing and
      # merge. Fires only when an agent-id is present; missing-agent-id
      # falls through to the specific error below.
      ...
      # Token-present check: a load-bearing APPROVE without a named
      # reviewer is a self-tagged approval — independent review never
      # happened. Closes PR #222 / #224 pattern observed 2026-06-10.
      ...
      # Canonical agent-id allowlist per CLAUDE.md 'TDD + review'. Real
      # reviewer agent IDs match one of two shapes:
      #   (a) harness shape `^a[0-9a-f]{16}$` (17-char hex)
      #   (b) named-subagent shape `^cavecrew-reviewer-[a-z0-9-]+$`
      # Any other value is treated as a self-tag escape and rejected.
      if ! echo "$REVIEWER_AGENT_ID" | grep -qE '^(a[0-9a-f]{16}|cavecrew-reviewer-[a-z0-9-]+)$'; then
        ...
        echo "  Allowed shapes: harness ID '^a[0-9a-f]{16}\$' OR named subagent '^cavecrew-reviewer-<slug>\$'." >&2
        echo "  Self-tag escapes rejected per CLAUDE.md 'Never self-approve'." >&2
